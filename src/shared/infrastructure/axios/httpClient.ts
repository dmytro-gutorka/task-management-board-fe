...
import { ACCESS_TOKEN_KEY, API_ROUTES } from '../auth/auth-api.constants.ts';
...
httpClient.interceptors.response.use(
    ...
    async (error: AxiosError) => {
        ...
        const isRefreshRequest = originalRequest?.url?.includes(API_ROUTES.REFRESH);
        const isLogoutRequest = originalRequest?.url?.includes(API_ROUTES.LOGOUT);
